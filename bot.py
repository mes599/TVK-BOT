...
# History check limit for existing panels
MAX_PANEL_HISTORY = 50

# Discord channel names have a maximum length
...
# Prevents multiple simultaneous ticket creations.
...
def get_topic_value(
    ...
    """
    Reads a value from a structured channel topic.
...
class PaymentConfirmationModal(
    ...
    async def on_submit(
        ...
        embed.set_footer(
            text=(
                "Payment must be manually verified "
                "by the bot owner."
            )
        )

        # -------------------------------------------------
        # OWNER VERIFICATION VIEW
        # -------------------------------------------------
...
class PaymentVerifyButton(
    ...
    async def callback(
        ...
        if get_ticket_owner_id(channel) != self.customer_id:
        ...
        try:
            ...
            if interaction.message is not None:
                ...
                updated_embed.set_footer(
                    text=(
                        "Verified by the configured bot owner."
                    )
                )
            ...
            print(
                f"⚠️ Could not update owner message: {error}"
            )
        ...
        try:
            ...
            print(
                f"⚠️ Could not send verification message: {error}"
            )
        ...
        try:
            ...
            print(
                "⚠️ The customer has DMs disabled."
            )
...
class PaymentRejectButton(
    ...
    async def callback(
        ...
        try:
            ...
            print(
                f"⚠️ Could not send rejection message: {error}"
            )
        ...
        try:
            ...
            await channel.send(
                content=(
                    ...
                    "number of requested features and the "
