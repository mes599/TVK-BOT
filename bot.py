...
# Number of old messages checked for panels
...
# Railway provides DISCORD_BOT_TOKEN.
# Never put the real token directly into this file.
...
def is_ticket_channel(
    ...
    """
    Returns True for normal support/order channels.
    """
...
def get_topic_value(
    ...
    """
    Reads a value from a structured topic.

    Example:
    ticket-owner:123456 | ticket-type:order
    """
...
def build_ticket_topic(
    ...
    """
    Payment status is intentionally NOT stored in the topic.
    This avoids unnecessary channel PATCH requests.
    """
...
def get_payment_status_from_message(
    ...
    """
    Payment status is determined from the current
    owner payment message.
    """
...
class PaymentRejectionModal(
    ...
    async def on_submit(
        ...
        reason = (
            self.rejection_reason.value.strip()
        )
        ...
        try:
            ...
            async with lock:
                ...
                if ticket_owner_id != self.customer_id:
                ...
                try:
                    ...
                    print(
                        f"❌ Could not edit payment message: "
                        f"{error}"
                    )
                ...
                try:
                    ...
                    print(
                        f"⚠️ Could not send rejection message "
                        f"to ticket: {error}"
                    )
