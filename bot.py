...
# Name that will be shown in ticket messages
OWNER_DISPLAY_NAME = "mes 599"

# Panel search limit
MAX_PANEL_HISTORY = 50

# Discord maximum channel name length
...
def create_verified_embed(
    ...
    embed = discord.Embed(
        ...
        description=(
            f"This payment has been manually verified "
            f"by **{OWNER_DISPLAY_NAME}**."
        ),
...
def create_rejected_embed(
    ...
    embed = discord.Embed(
        ...
        description=(
            f"This payment has been rejected "
            f"by **{OWNER_DISPLAY_NAME}**."
        ),
...
async def edit_original_payment_message(
    client: discord.Client,
    message_id: int,
    embed: discord.Embed
) -> bool:
    ...
    try:
        ...
        message = await dm_channel.fetch_message(
            message_id
        )
        ...
        print(
            "⚠️ Original payment message was not found."
        )

    except discord.Forbidden:

        print(
            "⚠️ Cannot access owner's payment message."
        )

    except discord.HTTPException as error:

        print(
            f"⚠️ Could not edit payment message: {error}"
        )
...
class PaymentConfirmationModal(
    ...
    async def on_submit(
        ...
        payment_method_value = (
            self.payment_method.value.strip()
        )
        ...
        # This footer stores the information required
        # by Verify / Reject.
        embed.set_footer(
            text=(
                f"payment-request:"
        ...
        owner_notified = await send_owner_dm(
            ...
            content=(
                ...
                "Please check the payment method and "
...
class PaymentVerifyButton(
    ...
    async def callback(
        ...
        try:
            ...
            # ---------------------------------------------
            # READ DATA FROM PAYMENT MESSAGE
            # ---------------------------------------------
            ...
            # ---------------------------------------------
            # CHECK CURRENT PAYMENT MESSAGE STATUS
            # ---------------------------------------------

            current_status = get_payment_status_from_message(
                interaction.message
            )
            ...
            async with lock:
                ...
                try:
                    ...
                    print(
                        "⚠️ Could not update owner "
                        f"payment message: {error}"
                    )
                ...
                try:
                    ...
                    print(
                        f"⚠️ Could not send verification "
                        f"message: {error}"
                    )
...
class PaymentRejectionModal(
    ...
    async def on_submit(
        ...
        try:
            ...
            # ---------------------------------------------
            # GET ORIGINAL OWNER PAYMENT MESSAGE
            # ---------------------------------------------
            ...
            payment_message = await dm_channel.fetch_message(
            ...
            print(
                "❌ Unexpected error in Reject Payment:"
            )
    ...
    async def _get_guild(
        ...
        try:
            ...
            return await interaction.client.fetch_guild(
                self.guild_id
            )
