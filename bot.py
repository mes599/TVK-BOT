...
# Name shown in customer-facing verification messages.
OWNER_DISPLAY_NAME = "mes 599"
...
# Same tiers as PACKAGES, but for AI agents.
# Each price is exactly 10 higher than the matching bot package.
AGENT_PACKAGES = {
    "starter": {
        "name": "Starter Agent",
        "price": "€14.99",
...
# The real token is NOT stored in this file.
# Railway should provide DISCORD_BOT_TOKEN.
...
def build_ticket_topic(
    ...
    """
    Payment status is deliberately NOT stored in the topic.
    This avoids the channel PATCH/rate-limit problem.
    """
...
def get_payment_lock(
    ...
    lock = payment_action_locks.get(channel_id)

    if lock is None:
        lock = asyncio.Lock()
        payment_action_locks[channel_id] = lock
...
def get_payment_status_from_message(
    ...
    """
    Payment status is determined from the current payment
    message in the owner's DM.
    """
    ...
    if title == "💰 Payment Confirmation":
        return "reported"

    if title == "✅ Payment Verified":
        return "verified"

    if title == "❌ Payment Rejected Successfully":
        return "rejected"
...
async def fetch_ticket_channel(
    client: discord.Client,
    guild_id: int,
    channel_id: int
    ...
    try:
        ...
        fetched_channel = await guild.fetch_channel(
            channel_id
        )
        ...
        print(
            "❌ Permission denied while fetching ticket."
        )
        ...
        print(
            f"❌ Could not fetch ticket channel: {error}"
        )
...
# =========================================================
# PAYMENT VERIFIED EMBED
# =========================================================

def create_verified_embed(
    guild: discord.Guild,
    channel: discord.TextChannel,
    customer_id: int
) -> discord.Embed:
    ...
    embed = discord.Embed(
        ...
        description=(
            f"This payment has been verified by "
...
# =========================================================
# PAYMENT REJECTED EMBED
# =========================================================

def create_rejected_embed(
    guild: discord.Guild,
    channel: discord.TextChannel,
    customer_id: int,
    ...
    embed = discord.Embed(
        ...
        description=(
            f"This payment has been rejected by "
            f"**{OWNER_DISPLAY_NAME}**."
        ),
...
class PaymentConfirmationModal(
    ...
    account_name = discord.ui.TextInput(
        ...
        placeholder=(
            "Name of your Revolut or PayPal account"
        ),
        style=discord.TextStyle.short,
        required=True,
        max_length=100
    ...
    async def on_submit(
        ...
        payment_method_value = clean_payment_method(
        ...
        owner_notified = await send_owner_dm(
            ...
            content=(
                ...
                "Check the payment method and the "
                "reported account name before deciding."
            ),
            embed=embed,
        ...
        if owner_notified:
            ...
            await interaction.followup.send(
                (
                    "⚠️ Your payment information was saved, "
                    "but the owner could not be notified."
                ),
                ephemeral=True
            )
        ...
        try:
            ...
            await channel.send(
                (
                    ...
                    "and must manually verify the payment."
                )
            )
            ...
            print(
                "⚠️ Cannot send payment message in ticket."
            )
...
class PaymentVerifyButton(
    ...
    async def callback(
        ...
        try:
            ...
            guild_id, channel_id, customer_id = data

            # ---------------------------------------------
            # GET TICKET
            # ---------------------------------------------

            channel = await fetch_ticket_channel(
                interaction.client,
                guild_id,
                channel_id
            )
            ...
            current_status = (
                get_payment_status_from_message(
                    interaction.message
                )
            )
            ...
            lock = get_payment_lock(channel.id)
            ...
            async with lock:
                ...
                verified_embed = create_verified_embed(
                    guild=guild,
                    channel=channel,
                ...
                try:
                    ...
                    print(
                        f"⚠️ Could not update payment message: "
                        f"{error}"
                    )
                ...
                try:
                    ...
                    await channel.send(
                        (
                            ...
                            f"verified by {OWNER_DISPLAY_NAME}.**\n\n"
                    ...
                    print(
                        f"⚠️ Verification ticket message "
                        f"error: {error}"
                    )
            ...
            print(
                "❌ Unexpected error in Verify Payment:"
            )
...
class PaymentRejectionModal(
    ...
    def __init__(
        ...
        self.payment_message_id = payment_message_id
    ...
    async def on_submit(
        ...
        try:
            ...
            async with lock:
                ...
                # -----------------------------------------
                # FIND ORIGINAL PAYMENT MESSAGE
                # -----------------------------------------

                try:

                    payment_message = (
                        await owner_dm.fetch_message(
                            self.payment_message_id
                        )
                    )
                ...
                rejected_embed = create_rejected_embed(
                    guild=guild,
                    channel=channel,
                ...
                # -----------------------------------------
                # REPLACE ORIGINAL PAYMENT MESSAGE
                # -----------------------------------------
                ...
                try:
                    ...
                    print(
                        f"❌ Could not update payment message: "
                        f"{error}"
                    )
                ...
                try:
                    ...
                    await channel.send(
                        (
                            "❌ **Your payment has been "
                            f"rejected by {OWNER_DISPLAY_NAME}.**\n\n"

                            f"📝 **Reason:**\n"
                            f"{reason}\n\n"

                            "The server owner will contact "
                            "you later."
                        )
                    )
                    ...
                    print(
                        ...
                        "inside the private ticket."
                    )

                except discord.HTTPException as error:

                    print(
                        f"⚠️ Could not send rejection "
                        f"message: {error}"
                    )
            ...
            print(
                "❌ Unexpected error in PaymentRejectionModal:"
            )
        ...
        try:
            ...
            await channel.send(
                content=(
                    ...
                    "The exact development time depends "
                    "on the complexity of your bot, the "
                    "number of requested features and "
...
class AgentOrderModal(
    discord.ui.Modal,
    title="Order an Agent"
):

    idea = discord.ui.TextInput(
        label="Describe your idea",
        placeholder="What kind of agent do you want?",
        style=discord.TextStyle.paragraph,
        required=True,
        max_length=1000
    )
    ...
    async def on_submit(
        ...
        try:
            ...
            await channel.edit(
                topic=build_ticket_topic(
                    owner_id=user.id,
                    ticket_type="order-agent"
                ),
                reason="Ticket converted to agent order"
            )
        ...
        try:
            ...
            await channel.send(
                content=(
                    ...
                    "The exact development time depends "
                    "on the complexity of your agent, the "
                    "number of requested features and "
                    "the overall requirements.\n\n"
...
class AgentPackageButton(
    ...
    @discord.ui.button(
        label="🟢 Starter Agent — €14.99",
        style=discord.ButtonStyle.green,
        custom_id="package_agent_starter"
    )
    async def starter(
        self,
        interaction: discord.Interaction,
        ...
        await interaction.response.send_modal(
            AgentOrderModal(
                AGENT_PACKAGES["starter"]["name"],
                AGENT_PACKAGES["starter"]["price"]
            )
        )

    @discord.ui.button(
        label="🔵 Pro Agent — €19.99",
        style=discord.ButtonStyle.blurple,
        custom_id="package_agent_pro"
    )
    async def pro(
        self,
        interaction: discord.Interaction,
        ...
        await interaction.response.send_modal(
            AgentOrderModal(
                AGENT_PACKAGES["pro"]["name"],
                AGENT_PACKAGES["pro"]["price"]
            )
        )

    @discord.ui.button(
        label="🟣 Advanced Agent — €24.99",
        style=discord.ButtonStyle.gray,
        custom_id="package_agent_advanced"
    )
    async def advanced(
        self,
        interaction: discord.Interaction,
        ...
        await interaction.response.send_modal(
            AgentOrderModal(
                AGENT_PACKAGES["advanced"]["name"],
                AGENT_PACKAGES["advanced"]["price"]
            )
        )
...
class TicketTypeButton(
    ...
    async def order_agent(
        ...
        embed.add_field(
            name="🟢 STARTER AGENT — €14.99",
            value=(
                "*Great for a simple, friendly AI assistant*\n\n"

                "💬 **Natural Conversations**\n"
                "Chats with your members using ChatGPT.\n\n"

                "🎭 **Custom Personality**\n"
                "Tone, style and behaviour tailored to your server.\n\n"

                "⚡ **Basic Commands**\n"
                "Simple commands to talk to and control the agent.\n\n"
        ...
        embed.add_field(
            name="🔵 PRO AGENT — €19.99",
            value=(
                "*Great for a smarter, more capable agent*\n\n"

                "✅ **Everything in Starter**\n"
                "Includes all Starter features.\n\n"

                "🧠 **Conversation Memory**\n"
                "Remembers recent context for more natural replies.\n\n"

                "🎯 **Custom Commands**\n"
                "Commands built specifically for your server.\n\n"
        ...
        embed.add_field(
            name="🟣 ADVANCED AGENT — €24.99",
            value=(
                "*Great for a fully autonomous, connected agent*\n\n"
                ...
                "🔄 **Automated Workflows**\n"
                "Let the agent carry out multi-step tasks on its own.\n\n"

                "🗄️ **Persistent Memory**\n"
                "Stores conversation history, preferences and data.\n\n"

                "🔐 **Advanced Permissions**\n"
                "Detailed control over what the agent can access and do.\n\n"
        ...
        embed.add_field(
            name="🚀 Ready to build your agent?",
            value=(
                "Choose your package below and tell us "
...
class TicketButton(
    ...
    async def create_ticket(
        ...
        try:
            ...
            ticket_type_embed = discord.Embed(
            ...
            ticket_type_embed.add_field(
                name="💻 Development",
                value=(
                    "Every custom bot and AI agent is built "
            ...
            ticket_type_embed.add_field(
                ...
                value=(
                    "Development usually takes **up to 5 days**. "
                    "The exact time depends on the complexity of "
                    "your project, how many features you request "
                    "and what it needs to be able to do."
                ),
                inline=False
            )
            ...
            ticket_type_embed.add_field(
                ...
                value=(
                    ...
                    "• You must add me to your server so I can set up and configure everything.\n"
                    "• Please make sure I have the necessary permissions to complete the setup.\n"
                    "• **AI Agent orders only:** you'll need to provide your own "
                    "OpenAI (ChatGPT) API key so the agent can run."
                ),
                inline=False
            )
            ...
            print(
                "❌ Cannot send messages in the new ticket."
            )
...
async def on_ready():
    ...
    if channel is None:
        ...
        panel_embed = discord.Embed(
            ...
            description=(
                ...
                "ℹ️ Development time and requirements are "
                "explained inside your ticket once it's opened."
            ),
            color=discord.Color.blurple()
        )
        ...
        try:
            ...
            print(
                "❌ I don't have permission to update "
                "the ticket panel."
            )
            ...
            print(
                f"❌ Could not update ticket panel: {error}"
            )
    ...
    try:
        ...
        if showcase_message is None:
            ...
            await showcase_message.edit(
                embed=showcase_embed,
                view=ShowcaseView()
            )

            print(
                "✅ Showcase panel updated successfully."
            )
        ...
        print(
            f"❌ Could not update showcase: {error}"
        )
