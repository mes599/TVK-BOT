...
# Number of messages checked when searching for existing panels
MAX_PANEL_HISTORY = 50

# Discord channel name maximum
...
# Token is NOT stored in this file.
# Railway provides DISCORD_BOT_TOKEN as an environment variable.
...
# Prevents multiple ticket creations from racing each other.
ticket_creation_lock = asyncio.Lock()

# Prevents two payment actions on the same ticket
...
def get_topic_value(
    ...
    """
    Reads structured information from a channel topic.
...
def get_payment_request_data(
    ...
    """
    Payment notification footer contains:
...
class PaymentConfirmationModal(
    ...
    async def on_submit(
        ...
        if current_status != "pending":
            ...
            await interaction.response.send_message(
                (
                    "⚠️ This payment has already been "
        ...
        if not status_updated:
            ...
            await interaction.followup.send(
                (
                    "❌ I could not save the payment status. "
        ...
        # -------------------------------------------------
        # IMPORTANT:
        # These IDs allow Verify/Reject to locate the
        # correct ticket even though the buttons themselves
        # use persistent/static custom IDs.
        # -------------------------------------------------
        ...
        owner_notified = await send_owner_dm(
            ...
            content=(
                ...
                "Please check the payment method and the "
                "reported account name before verifying."
            ),
            embed=embed,
        ...
        if owner_notified:
            ...
            await interaction.followup.send(
                (
                    "⚠️ Your payment report was saved, "
        ...
        try:
            ...
            if data is None:
                ...
                await interaction.followup.send(
                    (
                        ...
                        "request information from this message."
                    ),
                    ephemeral=True
                )
            ...
            async with lock:
                ...
                try:
                    ...
                    print(
                        "⚠️ Verification ticket message error: "
                        f"{error}"
                    )
            ...
            print(
                f"❌ Unexpected error in Verify Payment: {error}"
            )
        ...
        try:
            ...
            async with lock:
                ...
                try:
                    ...
                    print(
                        "⚠️ Rejection ticket message error: "
                        f"{error}"
                    )
            ...
            print(
                f"❌ Unexpected error in Reject Payment: {error}"
            )
...
class TicketCloseButton(
    ...
    async def close_ticket(
        ...
        # Only ticket owner or bot owner.
        if (
